...
class TolokaProcessingApi:
    ...
    def create_toloka_project(self, project_params):
        req = requests.post("https://toloka.dev/api/v1/projects", headers=self.headers, json=project_params)
        assert (req.ok)
        new_project_id = req.json()['id']
        print("New project was created. New project id: ", new_project_id)
        print("https://toloka.dev/requester/project/{}".format(new_project_id))
        return new_project_id

    def create_toloka_pool(self, project_id, pool_params):
        pool_params['project_id'] = str(project_id)
        req = requests.post("https://toloka.dev/api/v1/pools", headers=self.headers, json=pool_params)
        assert (req.ok)
    ...
    def clone_toloka_pool(self, pool_id):
        req = requests.post("https://toloka.dev/api/v1/pools/{}/clone".format(pool_id), headers=self.headers)
        assert (req.ok)
        operation_id = req.json()['id']
        time.sleep(10)
        operation_status = requests.get("https://toloka.dev/api/v1/operations/{}".format(operation_id),
        ...
        print("https://toloka.dev/requester/pool/{}".format(new_pool_id))
        return new_pool_id

    def upload_toloka_tasks(self, tasks):
        req = requests.post("https://toloka.dev/api/v1/task-suites?allow_defaults=true",
    ...
    def run_toloka_pool(self, pool_id):
        req = requests.post("https://toloka.dev/api/v1/pools/{}/open".format(pool_id), headers=self.headers)
        assert (req.ok)
        print("Pool {} was started".format(pool_id))

    def print_toloka_pool_link(self, pool_id):
        print("https://toloka.dev/requester/pool/{}".format(pool_id))

    def get_pool_params(self, pool_id):
        req = requests.get("https://toloka.dev/api/v1/pools/{}".format(pool_id), headers=self.headers)
        assert req.ok
        return req.json()

    def update_pool_params(self, pool_id, pool_params):
        req = requests.put("https://toloka.dev/api/v1/pools/{}".format(pool_id), headers=self.headers,
                           json=pool_params)
        assert req.ok

    def get_pool_completed_tasks(self, pool_id):
        req = requests.get("https://toloka.dev/api/v1/assignments?pool_id={}&limit=10000".format(pool_id),
